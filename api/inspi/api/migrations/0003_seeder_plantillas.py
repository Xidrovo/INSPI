...
def crearPlantillas(apps, schema_editor):
	...
	dato_seleccion_unica = TipoDeDato_model.objects.filter(pk='seleccion_unica').first()
	dato_texto = TipoDeDato_model.objects.filter(pk='texto').first()
	dato_rango = TipoDeDato_model.objects.filter(pk='rango').first()
	dato_valor_exacto = TipoDeDato_model.objects.filter(pk='valor_exacto').first()
	dato_selecion_multiple = TipoDeDato_model.objects.filter(pk='seleccion_multiple').first()
	...
	pregunta_1a1.detalle = '[{"value": 100, "name": "Opción 1"}, {"value": 200, "name": "Opción 2"}, {"value": 300, "name": "Opción 3"}]'
	...
	pregunta_2a2.detalle = '[{"value": "ABC", "name": "Sepa 1"}, {"value": "DEF", "name": "Sepa 2"}, {"value": "GHI", "name": "Sepa 3"}]'
