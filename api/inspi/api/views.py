...
def crearPlantilla(request):
	if request.method == "POST":
		...
		plantilla = request.POST.get("PLANTILLA", None)

		#TRANSFORMAR STRING A DICCIONARIO
		plantillaJSON = ast.literal_eval(plantilla)
		...
		for seccion in secciones:
			...
			for pregunta in preguntas:
				...
				tipo_de_dato = pregunta["TIPO_DATO"]

				#BUSCAR EL TIPO DE DATO
				try:
					tipo_de_dato = TipoDeDato.objects.filter(nombre=tipo_de_dato).first()
				except: #SI NO ENCUENTRA EL TIPO DE DATO SE RETORNA UN TIPO DE DATOS 'DESCONOCIDO'
					tipo_de_dato = TipoDeDato().objects.filter(nombre="DESCONOCIDO").first()
